...
class LinearDistributionWidget(ScriptedLoadableModuleWidget):
  ...
  def onNeedlePlanButtonClicked(self):
    logic = LinearDistributionLogic()
    entryFiducial1 = self.entry1.value
    endFiducial1 = self.end1.value
    result = logic.setNeedlePlan(entryFiducial1, endFiducial1)
  ...
  def setup(self):
    ...
    entry1Label= qt.QLabel("Entry Point 1 = Fiducial ")
    entry1 = qt.QSpinBox()
    entry1.setMinimum(1)
    parametersFormLayout.addWidget(entry1Label)
    parametersFormLayout.addWidget(entry1)
    self.entry1 = entry1

    end1Label = qt.QLabel("Needle Tip 1 = Fiducial ")
    end1 = qt.QSpinBox()
    end1.setMinimum(1)
    parametersFormLayout.addWidget(end1Label)
    parametersFormLayout.addWidget(end1)
    self.end1 = end1


    #NEEDLE PLAN BUTTON
    needleButton = qt.QPushButton("Set Needle Plan")
...
class LinearDistributionLogic(ScriptedLoadableModuleLogic):
  ...
  def radialDose(self,fid, doseMap, vol, RastoIjkMatrix,originalPointinRAS):

    rad = max(20*doseMap[0])
    # tumor=[1,1,1]
    cube = []
    center = originalPointinRAS
    for i in range(-rad, rad + 1):
      for j in range(-rad, rad + 1):
        for k in range(-rad, rad + 1):
          pos = np.array([i, j, k])
          pt = pos + np.array(originalPointinRAS)
          pt = pt.tolist()

          for sphereradindex in range(len(doseMap[0])):
            if self.checkinSphere(pt, doseMap[0][sphereradindex], center):
              pt = np.append(pt, 1)
              pt=RastoIjkMatrix.MultiplyDoublePoint(pt)
              vol[int(pt[2]), int(pt[1]), int(pt[0])] += 1000 * doseMap[1][int(sphereradindex)]
  ...
  def calcDoseRadius(self, Image, cloneVolume):
    ...
    for i in range(num):
      ...
      print p_IJK
      self.radialDose(p_IJK[0:3], doseMap, vol, RastoIjkMatrix,originalPointinRAS)
